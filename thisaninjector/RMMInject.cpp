...
public:
    ...
    bool HideThread(HANDLE hThread) {
        if (!fnNtSetInformationThread) return false;
        
        NTSTATUS status = fnNtSetInformationThread(
            hThread,
    ...
    bool ReviveThreads(bool forceNewThreads = false) {
                ...
                if (!GetExitCodeThread(hThread, &exitCode) || exitCode != STILL_ACTIVE) {
                    ...
                    if (fnNtGetContextThread && fnNtSetContextThread) {
                        ...
                        if (NT_SUCCESS(fnNtGetContextThread(hThread, &ctx))) {
                            // Thread is probably suspended if we can get context but...
                            // Try to modify RIP slightly and see if it sticks (suspended threads allow this)
                            uintptr_t originalRip = ctx.Rip;
                            ctx.Rip += 0; // Just read, don't actually modify
                            
                            if (NT_SUCCESS(fnNtSetContextThread(hThread, &ctx))) {
                                // Thread might be suspended, resume it
        ...
        // If forced or we have too few active threads, create more
        if (forceNewThreads || m_ManagedThreads.size() < 3) {
            for (int i = 0; i < 3 - m_ManagedThreads.size(); i++) {
                HANDLE hNewThread = CreateHiddenThread();
                if (hNewThread) {
                    createdAny = true;
                }
...
namespace nbeater678_bypass {
	constexpr auto nbeater678_shift = Offsets::kPageShift;
	constexpr auto nbeater678_magic = Offsets::kPageHash;
	constexpr auto nbeater678_mask = 0xFFFF;
	constexpr auto nbeater678_align = Offsets::cfg_cache_alignment;

	__forceinline uintptr_t nbeater678_crypt(uintptr_t addr) {
		return ((addr >> nbeater678_shift) ^ nbeater678_magic);
	}
	...
	void nbeater678_whitelist(HANDLE hProc, uintptr_t base, size_t len) {
		...
		auto fn = reinterpret_cast<uintptr_t>(mod) + Offsets::set_insert;
		auto map = reinterpret_cast<uintptr_t>(mod) + Offsets::map;
		uintptr_t low = base & ~0xFFF, high = (base + len + 0xFFF) & ~0xFFF;
		BYTE flag = 0; ReadProcessMemory(hProc, (void*)(__readgsqword(0x30) + 0x2EC), &flag, 1, 0);
		if (!(flag & 0x10)) {
			BYTE n = flag | 0x10;
			WriteProcessMemory(hProc, (void*)(__readgsqword(0x30) + 0x2EC), &n, 1, 0);
		}
		...
		for (uintptr_t p = low; p < high; p += 0x1000) {
			...
			for (int t = 0; t < 10 && !ok; ++t) {
				ok = SyscallRemoteCall(hProc, fn, map, 0xf40ce68d, h);
				if (!ok) Sleep(50);
			}
			if (!ok) break;
		}
		...
		BYTE f = flag & ~0x10;
		WriteProcessMemory(hProc, (void*)(__readgsqword(0x30) + 0x2EC), &f, 1, 0);
	}
	...
	void nbeater678_cache(HANDLE hProc, uintptr_t base, size_t len) {
		...
		base &= ~nbeater678_mask;
		len = ((len + nbeater678_align - 1) / nbeater678_align) * nbeater678_align;

		for (uintptr_t o = 0; o < len; o += nbeater678_align) {
			uintptr_t pg = base + o;
			auto entry = cbase + (pg >> 0x13);
			auto bit = 1 << (((pg >> 0x10) & 7) % 32);
			uint32_t cur = 0;
			ReadProcessMemory(hProc, (void*)entry, &cur, 4, 0);
			cur |= bit;
			WriteProcessMemory(hProc, (void*)entry, &cur, 4, 0);
		}
	}
}
    ...
    for (uintptr_t addr = start; addr < end; addr += 0x10000) {
        ...
        if (VirtualQueryEx(hProcess, (LPVOID)addr, &mbi, sizeof(mbi))) {
            if (mbi.Protect & PAGE_EXECUTE) {
                anyExec = true;
                printf("[+] Verified executable at %p: 0x%X\n", (void*)addr, mbi.Protect);
                break;
            }
        }
    }
...
#pragma region Write file into process
    ...
    // Allocate memory initially as RW (not RWX) to avoid suspicion
    uintptr_t TargetBase = Process::details::RemoteAlloc<uintptr_t>(proc._handle, OptHeader->SizeOfImage, PAGE_READWRITE);
...
#pragma endregion
    ...
    // 4 & 5: Fallback direct syscall protect grouped by section
    if (!allOk) {
        printf("[*] Fallback: direct syscall protect for executable sections...\n");
        uintptr_t fnIns = loader.Start + Offsets::set_insert;
        uintptr_t mapAd = loader.Start + Offsets::map;
        for (auto sec : execSecs) {
            uintptr_t secBase = TargetBase + sec->VirtualAddress;
            size_t secSize = sec->Misc.VirtualSize ? sec->Misc.VirtualSize : sec->SizeOfRawData;
            uintptr_t start = secBase & ~0xFFF;
            uintptr_t end = (secBase + secSize + 0xFFF) & ~0xFFF;
            for (uintptr_t p = start; p < end; p += 0x1000) {
                uint64_t hsh = ((p >> Offsets::kPageShift) ^ Offsets::kPageHash);
                bool ok = false;
                for (int i = 0; i < 5; ++i) {
                    ok = SyscallRemoteCall(proc._handle, fnIns, mapAd, 0xf40ce68d, hsh);
                    if (ok) break;
                    Sleep(20);
                }
                if (!ok) printf("[!] Syscall protect failed at %p\n", (void*)p);
            }
        }
    }
    ...
    for (auto sec : execSecs) {
        ...
        MEMORY_BASIC_INFORMATION mbi = {};
        if (VirtualQueryEx(proc._handle, reinterpret_cast<LPCVOID>(testAddr), &mbi, sizeof(mbi))) {
    ...
    struct WLParams { HANDLE h; uintptr_t base; size_t size; std::vector<IMAGE_SECTION_HEADER*> secs; };
    auto* wl = new WLParams{ proc._handle, TargetBase, OptHeader->SizeOfImage, execSecs };
    HANDLE hWL = CreateThread(nullptr, 0, [](LPVOID p) -> DWORD {
        auto* pr = reinterpret_cast<WLParams*>(p);
        srand((unsigned)GetTickCount());
        while (true) {
            Sleep(5000 + rand() % 2000);
            for (auto sec : pr->secs) {
                uintptr_t b = pr->base + sec->VirtualAddress;
                size_t s = sec->Misc.VirtualSize ? sec->Misc.VirtualSize : sec->SizeOfRawData;
                nbeater678_bypass::nbeater678_whitelist(pr->h, b, s);
                nbeater678_bypass::nbeater678_cache(pr->h, b, s);
            }
        }
        return 0;
    }, wl, 0, nullptr);
    ...
    // CFG Cache operation - critical for proper execution
    printf("[*] Step 2: Updating CFG cache...\n");
    {
        uintptr_t cache_base = 0;
        if (ReadProcessMemory(proc._handle, reinterpret_cast<LPCVOID>(loader.Start + Offsets::cfg_cachee), &cache_base, sizeof(cache_base), nullptr) && cache_base) {
            // Set up proper masking and alignment
            uintptr_t aligned_base = TargetBase & ~Offsets::kPageMask;
            size_t aligned_size = ((OptHeader->SizeOfImage + Offsets::cfg_cache_alignment - 1) / Offsets::cfg_cache_alignment) * Offsets::cfg_cache_alignment;
            
            for (uintptr_t offset = 0; offset < aligned_size; offset += Offsets::cfg_cache_alignment) {
                uintptr_t page = aligned_base + offset;
                auto entry = cache_base + (page >> 0x13);
                auto bit = 1 << (((page >> 0x10) & 7) % 32);
                
                uint32_t current = 0;
                ReadProcessMemory(proc._handle, reinterpret_cast<LPCVOID>(entry), &current, sizeof(current), nullptr);
                current |= bit;
                WriteProcessMemory(proc._handle, reinterpret_cast<LPVOID>(entry), &current, sizeof(current), nullptr);
            }
        }
    }
    ...
    auto extraStack = std::vector<void*>{
        reinterpret_cast<void*>(loader.Start + Offsets::set_insert),
        reinterpret_cast<void*>(loader.Start + Offsets::map),
        reinterpret_cast<void*>(TargetBase),
        reinterpret_cast<void*>(_GetProcAddress),
        reinterpret_cast<void*>(_GetModuleHandleA),
        reinterpret_cast<void*>(_LoadLibraryA),
        reinterpret_cast<void*>(_MessageBoxA),
        reinterpret_cast<void*>(proc._handle)
    };
    ...
    printf("[*] Step 7: Starting protection guard...\n");
	{
		...
		IMAGE_DOS_HEADER dosHeader = {};
		IMAGE_NT_HEADERS ntHeaders = {};
		ReadProcessMemory(proc._handle, reinterpret_cast<LPCVOID>(TargetBase), &dosHeader, sizeof(dosHeader), nullptr);
		
		size_t imageSize = 0x1000000; // Default large size
		if (dosHeader.e_magic == IMAGE_DOS_SIGNATURE) {
			ReadProcessMemory(proc._handle, reinterpret_cast<LPCVOID>(TargetBase + dosHeader.e_lfanew), &ntHeaders, sizeof(ntHeaders), nullptr);
				...
				for (uintptr_t offset = scanOffset; offset < size; offset += 0x1000) {
					...
					if (VirtualQueryEx(hProcess, reinterpret_cast<LPCVOID>(addr), &mbi, sizeof(mbi))) {
						if ((mbi.Protect & PAGE_EXECUTE) == 0) {
                            ...
                            printf("[!] Guardian #1: Restored execution at %p\n", mbi.BaseAddress);
						}
					}
				}
				
                Sleep(25); // More frequent checking
			}
        ...
        HANDLE hThread2 = CreateThread(nullptr, 0, [](LPVOID param) -> DWORD {
            ...
            IMAGE_DOS_HEADER dosHeader = {};
            ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(base), &dosHeader, sizeof(dosHeader), nullptr);
            
            if (dosHeader.e_magic == IMAGE_DOS_SIGNATURE) {
                IMAGE_NT_HEADERS ntHeaders = {};
                ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(base + dosHeader.e_lfanew), &ntHeaders, sizeof(ntHeaders), nullptr);
                ...
                for (int i = 0; i < ntHeaders.FileHeader.NumberOfSections; i++) {
                    ...
                    ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(sectionOffset + i * sizeof(IMAGE_SECTION_HEADER)), 
            ...
            while (true) {
                for (const auto& section : sections) {
                    DWORD oldProtect;
                    BOOL result = VirtualProtectEx(hProcess, reinterpret_cast<LPVOID>(section.address), 
                    ...
                    if (result && oldProtect != section.protection) {
                        ...
                        for (size_t offset = 0; offset < section.size; offset += 0x1000) {
                            uint8_t byte;
                            if (ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(section.address + offset), 
                                                &byte, 1, nullptr)) {
                                WriteProcessMemory(hProcess, reinterpret_cast<LPVOID>(section.address + offset), 
                                                 &byte, 1, nullptr);
                            }
                        }
                        
                        // Ensure changes take effect
                        FlushInstructionCache(hProcess, reinterpret_cast<LPVOID>(section.address), section.size);
                    }
                }
        ...
        HANDLE hThread3 = CreateThread(nullptr, 0, [](LPVOID param) -> DWORD {
            ...
            printf("[*] Protection guardian #3 (CFG Cache) started for range: 0x%p - 0x%p\n", (void*)base, (void*)(base + size));
            ...
            if (!ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>((uintptr_t)robloxModule + Offsets::cfg_cachee), 
                        ...
                        if ((mbi.Protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY))) {
                            ...
                            for (uintptr_t page = (uintptr_t)mbi.BaseAddress; 
                                ...
                                if (!alreadyUpdated) {
                                    ...
                                    uint32_t current = 0;
                                    if (ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(entry), &current, sizeof(current), nullptr)) {
                                        // Set the bit if not already set
                                        if ((current & bit) == 0) {
                                            uint32_t newValue = current | bit;
                                            if (WriteProcessMemory(hProcess, reinterpret_cast<LPVOID>(entry), &newValue, sizeof(newValue), nullptr)) {
...
int main()
{
    ...
    bool injectionSuccess = ManualMap(proc, dllname);
    ...
    if (injectionSuccess) {
        ...
        HANDLE monitorThread = CreateThread(nullptr, 0, [](LPVOID param) -> DWORD {
            ...
            // Maintain thread protection maps
            struct ThreadInfo {
                DWORD id;
                DWORD lastSeen;
                bool wasExecutingDll;
            };
            std::vector<ThreadInfo> knownThreads;
            
            // Get accurate module size information to better protect it
		IMAGE_DOS_HEADER dosHeader = {};
		IMAGE_NT_HEADERS ntHeaders = {};
            ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(base), &dosHeader, sizeof(dosHeader), nullptr);
		
            size_t moduleSize = 0x1000000; // Default large size
		...
		if (dosHeader.e_magic == IMAGE_DOS_SIGNATURE) {
			...
			if (ntHeaders.OptionalHeader.SizeOfImage > 0) {
                    ...
                    entryPoint = base + ntHeaders.OptionalHeader.AddressOfEntryPoint;
                    printf("[*] Monitor: Module size: 0x%zx, Entry point: 0x%p\n", moduleSize, (void*)entryPoint);
                }
            }
            
            // Counter for thread management operations
            int threadMgmtCounter = 0;
            
            // Super aggressive protection loop
			while (true) {
                bool foundViolation = false;
                bool didWhitelist = false;
				
                // PART 1: Check and maintain memory protections
					MEMORY_BASIC_INFORMATION mbi = {};
                for (uintptr_t addr = base; addr < base + moduleSize; addr += mbi.RegionSize) {
					if (VirtualQueryEx(hProcess, reinterpret_cast<LPCVOID>(addr), &mbi, sizeof(mbi))) {
                        // Focus on committed memory that should be executable
                        if (mbi.State == MEM_COMMIT) {
                            // Check if execution is blocked
						if ((mbi.Protect & PAGE_EXECUTE) == 0) {
                                // Found a protection violation
                                foundViolation = true;
                                totalViolations++;
                                lastViolationTime = GetTickCount();
                                
                                // Fix it aggressively with RWX
							DWORD oldProtect;
                                VirtualProtectEx(hProcess, mbi.BaseAddress, mbi.RegionSize, 
                                               PAGE_EXECUTE_READWRITE, &oldProtect);
							
                                // Touch memory to force permissions to apply
                                for (uintptr_t offset = 0; offset < mbi.RegionSize; offset += 0x100) {
                                    uint8_t buffer[16];
							SIZE_T bytesRead = 0;
                                    if (ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>((uintptr_t)mbi.BaseAddress + offset), 
                                                        buffer, sizeof(buffer), &bytesRead) && bytesRead > 0) {
                                        WriteProcessMemory(hProcess, reinterpret_cast<LPVOID>((uintptr_t)mbi.BaseAddress + offset), 
                                                         buffer, bytesRead, nullptr);
                                    }
                                }
                                
                                // Flush instruction cache for the entire region
							FlushInstructionCache(hProcess, mbi.BaseAddress, mbi.RegionSize);
                                
                                printf("[!] Monitor: Fixed execution at %p (size: 0x%zx)\n", 
                                      mbi.BaseAddress, mbi.RegionSize);
                            }
                            
                            // Update CFG cache for this region regardless of protection status
                            if (cfg_cache_base && robloxModule) {
                                for (uintptr_t pageAddr = (uintptr_t)mbi.BaseAddress; 
                                     pageAddr < (uintptr_t)mbi.BaseAddress + mbi.RegionSize; 
                                     pageAddr += 0x1000) {
                                    
                                    uintptr_t page_aligned = pageAddr & ~Offsets::kPageMask;
                                    uintptr_t entry = cfg_cache_base + (page_aligned >> 0x13);
                                    uint32_t bit = 1 << (((page_aligned >> 0x10) & 7) % 32);
                                    ...
                                    if (ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(entry), &current, sizeof(current), nullptr)) {
                                        if ((current & bit) == 0) {
                                            uint32_t newValue = current | bit;
                                            WriteProcessMemory(hProcess, reinterpret_cast<LPVOID>(entry), &newValue, sizeof(newValue), nullptr);
                                        }
                                    }
                                }
                            }
                            
                            // Also periodically re-whitelist pages
                            if (!didWhitelist && robloxModule && (rand() % 10 == 0)) {
                                for (uintptr_t pageAddr = (uintptr_t)mbi.BaseAddress; 
                                     pageAddr < (uintptr_t)mbi.BaseAddress + mbi.RegionSize; 
                                     pageAddr += 0x1000) {
                                    
                                    uint64_t hash = ((pageAddr >> Offsets::kPageShift) ^ Offsets::kPageHash);
                                    uintptr_t fnAddr = (uintptr_t)robloxModule + Offsets::set_insert;
                                    uintptr_t mapAddr = (uintptr_t)robloxModule + Offsets::map;
                ...
                // PART 2: Thread management integration
                threadMgmtCounter++;
                if (threadMgmtCounter >= 20) {
                    threadMgmtCounter = 0;
                    
                    // Use the thread manager to maintain active threads
                    if (g_ThreadManager) {
                        // Monitor thread states and revive if needed
                        if (g_ThreadManager->ReviveThreads(false)) {
                            printf("[+] Monitor: Revived terminated threads\n");
                        }
                        
                        // Periodically re-camouflage threads to avoid detection patterns
                        if (rand() % 3 == 0) {
                            g_ThreadManager->CamouflageThreads();
                        }
                        
                        // Check for and fix thread contexts
                        g_ThreadManager->RestoreThreadContexts();
                    }
                }
                
                // PART 3: Thread protection - prevent thread termination or suspension
                HANDLE hThreadSnap = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, GetProcessId(hProcess));
                if (hThreadSnap != INVALID_HANDLE_VALUE) {
                    THREADENTRY32 te32;
                    te32.dwSize = sizeof(THREADENTRY32);
                    
                    // First pass: update our thread list and track execution state
                    if (Thread32First(hThreadSnap, &te32)) {
                        do {
                            if (te32.th32OwnerProcessID == GetProcessId(hProcess)) {
                                // Get thread context to see if it's executing our DLL
                                HANDLE hThread = OpenThread(THREAD_ALL_ACCESS, FALSE, te32.th32ThreadID);
                                if (hThread) {
                                    CONTEXT ctx = { 0 };
                                    ctx.ContextFlags = CONTEXT_CONTROL;
                                    
                                    bool isExecutingDll = false;
                                    if (GetThreadContext(hThread, &ctx)) {
                                        if (ctx.Rip >= base && ctx.Rip < base + moduleSize) {
                                            isExecutingDll = true;
                                            
                                            // Immediately check if suspended and resume
                                            DWORD suspendCount = SuspendThread(hThread);
                                            if (suspendCount > 0) {
                                                printf("[!] Monitor: Found suspended thread in DLL, resuming (id: %u)\n", te32.th32ThreadID);
                                                ...
                                                // Extra resume just in case
                                                ResumeThread(hThread);
                                            } else {
                                                // We suspended it for the check, resume it
                                                ResumeThread(hThread);
                                            }
                                        }
                                    }
                                    ...
                                    bool found = false;
                                    for (auto& thread : knownThreads) {
                                        if (thread.id == te32.th32ThreadID) {
                                            thread.lastSeen = GetTickCount();
                                            thread.wasExecutingDll = isExecutingDll;
                                            found = true;
                                            break;
                                        }
                                    }
                                    ...
                                    if (!found) {
                                        ...
                                        newThread.id = te32.th32ThreadID;
                                        newThread.lastSeen = GetTickCount();
                                        newThread.wasExecutingDll = isExecutingDll;
                                        knownThreads.push_back(newThread);
                                    }
                    ...
                    // Clean up old threads from tracking
                    for (auto it = knownThreads.begin(); it != knownThreads.end();) {
                        if (GetTickCount() - it->lastSeen > 5000) {
                            it = knownThreads.erase(it);
                        } else {
                            ++it;
                        }
                    }
                ...
                // PART 4: Periodically force threads to start in our DLL if none are running
                bool anyThreadsInDll = false;
                for (const auto& thread : knownThreads) {
                    if (thread.wasExecutingDll) {
                        anyThreadsInDll = true;
                        break;
                    }
                }
                
                // If no threads are executing in our DLL, try to restart execution
                if (!anyThreadsInDll && entryPoint != 0 && (GetTickCount() - lastViolationTime > 2000)) {
                    printf("[!] Monitor: No threads executing in DLL, attempting to restart...\n");
                    
                    // Try first with thread manager
                    if (g_ThreadManager && g_ThreadManager->ReviveThreads(true)) {
                        printf("[+] Monitor: Created new hidden threads at various entry points\n");
                    } else {
                        // Fall back to direct thread creation at entry point
                        DWORD threadId = 0;
                        HANDLE hThread = CreateRemoteThread(hProcess, nullptr, 0, 
                                                          (LPTHREAD_START_ROUTINE)entryPoint, 
                                                          (LPVOID)((uintptr_t)base), 0, &threadId);
                        if (hThread) {
                            printf("[+] Monitor: Created new execution thread (id: %u) at entry point 0x%p\n", 
                                  threadId, (void*)entryPoint);
                            CloseHandle(hThread);
                        }
                    }
                }
                ...
                if (foundViolation) {
                    ...
                    // Report periodic monitoring status
                    if (consecutiveSuccesses % 50 == 0) {
                        DWORD now = GetTickCount();
                        DWORD timeSinceLastViolation = now - lastViolationTime;
                        printf("[*] Monitor status: %zu total violations, last violation %u ms ago\n", 
                              totalViolations, timeSinceLastViolation);
                        
                        // Thread statistics
                        if (g_ThreadManager) {
                            size_t mgmtThreadCount = g_ThreadManager->GetActiveThreadCount();
                            std::vector<DWORD> dllThreads = g_ThreadManager->FindThreadsInDll();
                            printf("[*] Thread management: %zu managed threads, %zu executing in DLL\n", 
                                   mgmtThreadCount, dllThreads.size());
                        }
                    }
                }
                
                Sleep(5); // Very frequent checks (200 times per second)
            }
